...
// 為主科別按鈕添加事件監聽器
document.querySelectorAll('.department-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
        // 移除先前選中的科別的 .btn-selected 類別
        document.querySelectorAll('.department-btn').forEach(function(b) {
...
function showSubDepartments(department) {
    ...
    var subDepartmentsHtml = subDepartments.map(function(sub) {
        return `<button type="button" class="btn btn-secondary mt-2 sub-department-btn">${sub}</button>`;
    }).join('');
    ...
    // 為細分部門按鈕添加事件監聽器
    subDepartmentsContainer.querySelectorAll('.sub-department-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            // 移除其他按鈕的 .btn-selected 類別
            subDepartmentsContainer.querySelectorAll('.sub-department-btn').forEach(function(b) {
                b.classList.remove('btn-selected');
            });
...
document.querySelectorAll('#ward-btn .btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
        // 移除同一按鈕組中其他按鈕的 .btn-selected 類別
        document.querySelectorAll('#ward-btn .btn').forEach(function(b) {
            b.classList.remove('btn-selected');
        });
...
document.getElementById('next-step-button').addEventListener('click', function() {
    window.location.href = '/SetSchedule/Index'; // 更新這個 URL 為您想要跳轉的路徑
});

document.getElementById('next-step-button').addEventListener('click', function() {
    // 獲取選中的細分科別和病房
    var selectedSubDepartment = document.querySelector('.sub-department-btn.btn-selected')?.textContent || '未選擇';
    var selectedWard = document.querySelector('#ward-btn .btn-selected')?.textContent || '未選擇';

    // 構建 URL 和查詢參數
    var url = `/SetSchedule/Index?subdepartment=${encodeURIComponent(selectedSubDepartment)}&ward=${encodeURIComponent(selectedWard)}`;
